...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const kind = req.body.kind === 'website' ? 'website' : 'app';
    const result = await appStudio.generateAppHtml(prompt.slice(0, 500), kind);
    const now = new Date().toISOString();
    const app_ = await updateBusiness(req, (b) => {
      const a = appStudio.emptyApp(b.nextIds.apps++, result.name, kind);
      a.previewHtml = result.html;
      a.messages = [
        { role: 'user', text: prompt, ts: now },
        { role: 'assistant', text: result.reply, ts: now },
      ];
      a.log = [{ id: 1, text: 'Generated by AI.', createdAt: now }];
      b.apps.push(a);
      return a;
    });
    res.status(201).json(app_);
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const result = await appStudio.chatEditApp(existing, message);
    const now = new Date().toISOString();
    const updated = await updateBusiness(req, (b2) => {
      const a = appStudio.findApp(b2, req.params.id);
      if (!a) return null;
      if (!a.messages) a.messages = [];
      a.messages.push({ role: 'user', text: message, ts: now });
      if (result.html) a.previewHtml = result.html;
      a.messages.push({ role: 'assistant', text: result.reply, ts: new Date().toISOString() });
      appStudio.touch(a);
      appStudio.logEntry(a, `Chat: ${message.slice(0, 60)}`);
      return a;
    });
    if (!updated) return res.status(404).json({ error: 'App not found' });
    res.json({ app: updated, reply: result.reply });
  })
);
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const a = appStudio.findApp(b, req.params.id);
    if (!a) return res.status(404).json({ error: 'App not found' });
    const html = a.previewHtml || appStudio.exportAppHtml(a);
