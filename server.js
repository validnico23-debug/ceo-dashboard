...
app.post(
  ...
  asyncRoute(async (req, res) => {
    const template = appStudio.TEMPLATES[req.body.templateId];
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!prompt) return res.status(400).json({ error: 'Describe the app you want to build.' });
    const spec = await appStudio.generateAppSpec(prompt.slice(0, 500));
    const app_ = await updateBusiness(req, (b) => {
