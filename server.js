...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const app_ = await updateBusiness(req, (b) => {
      ...
      const screenCount = (a.screens || []).length;
      const screenNames = (a.screens || []).map((s) => s.name).join(', ');
      const pageWord = kind === 'website' ? 'page' : 'screen';
      const reply = `Built "${a.name}"${a.subtitle ? ' — ' + a.subtitle : ''}. ${screenCount} ${pageWord}${screenCount === 1 ? '' : 's'}${screenNames ? ': ' + screenNames : ''}. Tell me what to change.`;
      a.messages = [
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const updated = await updateBusiness(req, (b2) => {
      ...
      if (result.spec) appStudio.applySpec(b2, a, result.spec);
